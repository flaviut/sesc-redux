...
public:
    ...
    virtual bool isOnlyWrites(Addr_t paddr) {
        return false;
    }
    ...
    virtual bool readSEntryFull() {
        return false;
    }
    virtual bool writeSEntryFull() {
        return false;
    }

    Addr_t calcLineAddr(Addr_t paddr) const {
        return paddr >> Log2LineSize;
    }

    int32_t getnEntries() const {
        return nEntries;
    }

    virtual int32_t  getnReads() const {
        return 1;
    }
    virtual int32_t  getnWrites() const {
        return 1;
    }

    virtual int32_t  getUsedReads(Addr_t paddr) {
        return 0;
    }
    virtual int32_t  getUsedWrites(Addr_t paddr) {
        return 0;
    }
    ...
    virtual void setLowerCache(Cache_t *lCache) {
        lowerCache = lCache;
    }
...
public:
    ...
    int32_t getUsedReads()     const {
        return (nReads - nFreeReads);
    }
    int32_t hasFreeReads()     const {
        return (nFreeSEntries > 0 && nFreeReads > 0);
    }

    int32_t getNWrittenWords() const {
        return writeSet.size();
    }
    int32_t getUsedWrites()    const {
        return (nWrites - nFreeWrites);
    }
    bool hasFreeWrites()   const {
        return (nFreeSEntries > 0 && nFreeWrites > 0);
    }

    int32_t getPendingReqs()  const {
        return nReads+nWrites-nFreeSEntries;
    }
...
public:
    ...
    int32_t getnReads() const {
        return nReads;
    }
    int32_t getnWrites() const {
        return nWrites;
    }
...
public:
    ...
    int32_t getnReads() const {
        I(nBanks);
        return mshrBank[0]->getnReads();
    }
    int32_t getnWrites() const {
        I(nBanks);
        return mshrBank[0]->getnWrites();
    }
