...
void StridePrefetcher::learnHit(PAddr addr)
{
    ...
    LOG("SP:prefetching more: addr=%08x", paddr + pe->stride);
}
...
void StridePrefetcher::learnMiss(PAddr addr)
{
    ...
    if(!table->readLine(nextAddr) && !table->readLine(paddr)) {
        ...
        pe->stride = newStride;
        pe->goingUp = goingUp;
        LOG("SP: new stream. stride=%d paddr=%08x nextAddr=%08x %s", (int) newStride, paddr, nextAddr, goingUp ? "UP" : "DOWN");
...
void StridePrefetcher::access(MemRequest *mreq)
{
    ...
    if (mreq->getMemOperation() == MemRead
        ...
        LOG("SP:ignoring access addr=%08x type=%d", paddr, mreq->getMemOperation());
        nextBuffSlot();
