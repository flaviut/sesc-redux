...
Cluster::Cluster(const char *clusterName, GProcessor *gp)
        : window(gp, clusterName), MaxWinSize(SescConf->getInt(clusterName, "winSize")),
          windowSize(SescConf->getInt(clusterName, "winSize")), gproc(gp)
...
void Cluster::buildUnit(const char *clusterName, GMemorySystem *ms, Cluster *cluster, InstType type,
    ...
    switch (type) {
        ...
        case iDiv:
            ...
            eng = new GStatsEnergyCG(static_cast<const char *>(strtmp), name, gproc->getId(), ExecPower,
                                     EnergyMgr::get("iALUEnergy", gproc->getId()), ecgbase);
            free(strtmp);
        ...
        case fpDiv:
            ...
            eng = new GStatsEnergyCG(static_cast<const char *>(strtmp), name, gproc->getId(), ExecPower,
                                     EnergyMgr::get("fpALUEnergy", gproc->getId()), ecgbase);
            free(strtmp);
