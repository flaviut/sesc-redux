...
inline bool isGprName(RegName reg) {
    return (reg & RegTypeMask) == RegTypeGpr;
}

inline bool isFprName(RegName reg) {
    return (reg & RegTypeMask) == RegTypeFpr;
}

inline bool isCtlName(RegName reg) {
    return (reg & RegTypeMask) == RegTypeCtl;
}

inline bool isSpcName(RegName reg) {
    return (reg & RegTypeMask) == RegTypeSpc;
}

inline bool isNotName(RegName reg) {
    return (reg & RegTypeMask) >= NumOfRegs;
}

inline size_t getRegNum(RegName reg) {
    return (reg & RegNumMask);
}

inline RegName getRegType(RegName reg) {
    return (RegName) (reg & RegTypeMask);
}
