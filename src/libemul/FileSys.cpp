        ...
        if (keyIt == byNatKey.end()) {
            switch (stbuf.st_mode & S_IFMT) {
                ...
                case S_IFREG:
                    ...
                    break;
                case S_IFDIR:
                    node = new DirectoryNode(stbuf);
                    break;
                default:
                    fail("Node::lookup(%s) for unknown st_mode %x\n", name.c_str(), stbuf.st_mode);
            }
...
//      fail("FileSys::FileNode destructor could not close file %s\n",name.c_str());
    ...
    void OpenFiles::openDescriptor(fd_t fd, Description *description) {
        I(fd >= 0);
        std::pair<FileDescriptors::iterator, bool> res = fileDescriptors.insert(
                FileDescriptors::value_type(fd, FileDescriptor()));
