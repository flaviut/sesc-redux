...
public:
    static inline T getReg(const ThreadContext *context, RegName name) {
        ...
        uint32_t retVal = MipsRegAccess<mode, uint32_t, RegTypeGpr>::getReg(context,
                                                                            RegName(ArchMipsRegNames::RegFCSR));
        ...
        for (int32_t fcc = ArchMipsRegNames::FccNameLb; fcc != ArchMipsRegNames::FccNameUb; fcc++)
            if (MipsRegAccess<mode, bool, RegTypeGpr>::getReg(context, RegName(fcc)))
                retVal |= (uint32_t(1)
                        << (23 + (fcc - ArchMipsRegNames::FccNameLb) + (fcc != ArchMipsRegNames::FccNameLb)));
        return T(retVal | uint32_t(
                MipsRegAccess<mode, uint8_t, RegTypeGpr>::getReg(context, RegName(ArchMipsRegNames::FpRMode))));
    }

    static inline void setReg(ThreadContext *context, RegName name, T val) {
        I(sizeof(T) == sizeof(uint32_t));
        MipsRegAccess<mode, uint32_t, RegTypeGpr>::setReg(context, RegName(ArchMipsRegNames::RegFCSR),
                                                          uint32_t(val) & 0x17FFFFFC);
        for (int32_t fcc = ArchMipsRegNames::FccNameLb; fcc != ArchMipsRegNames::FccNameUb; fcc++) {
            bool fccval = (uint32_t(val) & (uint32_t(1)
                    << (23 + (fcc - ArchMipsRegNames::FccNameLb) + (fcc != ArchMipsRegNames::FccNameLb))));
